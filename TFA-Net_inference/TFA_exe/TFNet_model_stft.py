...
class Trainer:
    ...
    def __init__(self,data_path):
        skip_path = os.path.join('model', 'RED-Net.pth')
    ...
    def inference(self):
        ...
        bz = f['bz'][:]
        f.close()
        print(real_data2.shape)

        N=256
        signal_50dB2 = np.zeros([int(bz), 8, N//2,N]).astype(np.float32)
        signal_50dB2[:, 0:4,:,:] = (real_data2.astype(np.float32)).transpose((3,2,1,0))
        signal_50dB2[:, 4:8,:,:] = (imag_data2.astype(np.float32)).transpose((3,2,1,0))
