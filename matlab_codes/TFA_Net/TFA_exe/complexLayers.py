...
class bmrelu(Module):
    def __init__(self,in_channel):
        super(bmrelu, self).__init__()
        self.fully_conn=Linear(in_channel*2,in_channel)
        self.bn=BatchNorm1d(in_channel)
        self.acti=torch.nn.ReLU()
        self.acti2=torch.nn.Sigmoid()
        self.fully_conn2=Linear(in_channel,in_channel)

    def forward(self, incoming):
        in_channels = incoming.size(1)
        bsz=incoming.size(0)
        scales_n = torch.mean(torch.mean(torch.minimum(incoming, torch.zeros_like(incoming)),dim=3,keepdim=True), dim=2,
                                  keepdim=True)
        scales_p = torch.mean(torch.mean(torch.maximum(incoming,  torch.zeros_like(incoming)), dim=3, keepdim=True), dim=2,
                                  keepdim=True)
        scale = torch.cat((scales_n, scales_p), 1)
        scale=scale.view(bsz,-1)
        scale=self.fully_conn(scale)
        scale=self.acti(self.bn(scale))
        scale=self.fully_conn2(scale)
        scale=self.acti2(scale)
        scale=scale.view(bsz,in_channels,1,1)
        minv=torch.min(torch.min(incoming,dim=2,keepdim=True).values,dim=3,keepdim=True).values
        return torch.maximum(incoming-scale*minv,torch.zeros_like(incoming))
