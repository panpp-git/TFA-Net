...
def train_frequency_representation(args, fr_module, fr_optimizer, fr_criterion, fr_scheduler, train_loader, val_loader,
    ...
    for batch_idx, (clean_signal, target_fr,wgt) in enumerate(train_loader):
        if args.use_cuda:
            clean_signal, target_fr,wgt = clean_signal.cuda(), target_fr.cuda(),wgt.cuda()
        ...
        loss_l2=torch.pow((output_fr - target_fr), 2)*wgt
    ...
    for batch_idx, (noisy_signal, _, target_fr,wgt) in enumerate(val_loader):
        if args.use_cuda:
            noisy_signal, target_fr,wgt = noisy_signal.cuda(), target_fr.cuda(),wgt.cuda()
...
if __name__ == '__main__':
    ...
    # basic parameters
    parser.add_argument('--output_dir', type=str, default='./checkpoint/TFA-Net_model', help='output directory')
